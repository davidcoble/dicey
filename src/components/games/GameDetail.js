...
import {
  startAddPlayerToGame,
  startRemovePlayerFromGame,
} from "../../actions/games";
import { history } from "../../routers/AppRouter";
import {
  startAddGameToPlayer,
  startRemoveGameFromPlayer,
  startSetPlayerRollingGame,
...
export class GameDetail extends React.Component {
  ...
  viewGame = (e) => {
    e.preventDefault();
    this.props.startSetPlayerRollingGame({
      uid: this.state.uid,
      gid: this.state.id,
    });
    history.push("/rolls+");
    ...
    return (
      <div className="rowList">
        <div className="gameDetail-rowName">
          ...
          <div className="gameDeet">Box</div>
          <div className="gameDeet">Players</div>
        </div>
            ...
            <div className="rowList-gameDeet">
              ...
              <div>
                {this.props.players.find((player) => {
                  ...
                  <button className="buttonJoin" onClick={this.quitGame}>
                    Quit
                  </button>
                ) : (
                  <button className="buttonJoin" onClick={this.joinGame}>
                    Join
                  </button>
                )}
              </div>
            ...
            <div className="rowList">
              {this.props.players.length === 0 ? (
                ...
                this.props.players.map((player) => {
                  return (
                    <div key={player.uid} className="playerBox">
                      {" "}
                      {player.name}{" "}
                    </div>
                  );
        ...
        {this.props.players.find((player) => {
          ...
          <div className="rowList">
            <button className="buttonView" onClick={this.viewGame}>
              View
            </button>
            <button className="buttonView" onClick={this.editGame}>
              Edit
            </button>
            <button className="buttonView" onClick={this.deleteGame}>
              Delete
            </button>
          </div>
        ) : (
...
const mapDispatchToProps = (dispatch, props) => ({
  ...
  startSetPlayerRollingGame: (data) =>
    dispatch(startSetPlayerRollingGame(data)),
});
